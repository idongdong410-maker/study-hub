...
이 워크플로우는 NotebookLM (`notebooklm-py`)을 통해 **실제 교재 기반** 학습 컨텐츠를 자동 생성합니다.
...
notebooklm login   # 브라우저로 Google 로그인 → 세션 저장
```
...
반드시 아래 JSON 형식으로만 출력해:
{
  ...
  "concepts": [
    {"id":"concept-calc-{오늘날짜}","subject":"calculus","topic":"선택된 단원명","content":"## 핵심 개념\n정의와 원리를 2~3문장으로\n\n## 핵심 공식\nf'(x) = lim... 등 공식 나열\n\n## 풀이 포인트\n시험에서 주의할 점, 자주 나오는 유형\n\n## 예제\n간단한 예시 1개","date":"{오늘날짜}"}
  ]
}
...
CALC_JSON=$(python .agents/scripts/ask_notebooklm.py b7c4a509-91f3-42c7-9df4-cdc2891fa779 "$QUESTION")
```
...
ENG_JSON=$(python .agents/scripts/ask_notebooklm.py 6d79c301-01b6-4e9b-bd4e-70b96d3aff71 "$QUESTION")
```
...
KOR_JSON=$(python .agents/scripts/ask_notebooklm.py a7a7587b-bc92-4cdb-807c-a71a6756c274 "$QUESTION")
```

### 5단계: feed.json 병합
- `CALC_JSON`, `ENG_JSON`, `KOR_JSON` 각 응답에서 questions, cards, concepts를 추출합니다.
...
```bash
notebooklm login   # Google 재로그인 → 세션 갱신
```
